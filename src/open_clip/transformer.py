...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(
        ...
        assert (attn_mask is None) or (key_padding_mask is None)
    ...
    def forward(
        ...
        x = q_x + self.ls_1(self.attention(
            q_x=self.ln_1(q_x), k_x=k_x, v_x=v_x, attn_mask=attn_mask, key_padding_mask=key_padding_mask
        ))
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, key_padding_mask: Optional[torch.Tensor] = None):
        ...
        for r in self.resblocks:
            if self.grad_checkpointing and not torch.jit.is_scripting():
                ...
                x = checkpoint(r, x, None, None, attn_mask, key_padding_mask=key_padding_mask)
            else:
...
class TextTransformer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert pool_type in ('first', 'last', 'argmax', 'eot', 'none')
        self.output_tokens = output_tokens
    ...
    def forward(self, text):
        ...
        key_pad_mask = (text == self.pad_id) if self.mask_pad else None
        x = x + self.positional_embedding[:seq_len].to(cast_dtype)
        x = self.transformer(x, attn_mask=attn_mask, key_padding_mask=key_pad_mask)
