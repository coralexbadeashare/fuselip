...
class SimpleTokenizer(object):
    ...
    def is_empty(self, ids):
        """ Check if the tokenized representation is empty (only contains padding tokens) """
        # bot and eot tokens are != 0
        return (ids != self.pad_id).sum() <= 2 * ids.shape[0]
...
class HFTokenizer:
    ...
    def is_empty(self, ids):
        ...
        if isinstance(self.tokenizer, T5TokenizerFast):
            text_is_empty = (ids != self.tokenizer.pad_token_id).sum() == 0
        else:
    ...
    def decode(self, ids: torch.Tensor) -> str:
        """ Decode the tokenized representation back to string """
